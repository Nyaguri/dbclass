...
create table student_table(
    ...
    studentMoblie varchar(20),
    studentMajor varchar(30)
);
insert into student_table(id, studentNumber, studentName, studentMoblie, studentMajor)
...
-- 사번(ename)이 7654인 사원 조회
select * from emp where ename='7654';

-- 조건이 2가지 이상
-- 직급이 manager 이고 부서번호가 20인 사원
select * from emp where job='manager' and deptno='20';
-- 직급이 salesman 이거나 clerk인 사원
select *from emp where job='salesman' or ename='clerk';
...
select * from emp where ename like '%m'; -- allen, martin
...
-- salesman 급여 총합
select sum(sal) as '급여 총합' from emp where job = 'salesman';
select sum(sal) as '급여 총합' from emp where job = 'manager';
-- 평균 급여 조회
...
select job as '직급', round(avg(sal), 1) as '평균 급여', count(*) as '사원 수'
...
*/
select deptno as '부서 번호', round(avg(sal), 1) as '평균 급여', max(sal) as '최고 급여', 
	min(sal) as '최소 급여', count(*) as '사원 수'
		from emp 
			group by deptno;
...
select date_format(hiredate, '%Y') as '입사년도', deptno as '부서 번호', count(*) as '사원 수'
	from emp
		group by date_format(hiredate, '%Y'), deptno;
        
-- 조인(join)
-- 두 개 이상의 테이블을 하나의 테이블처럼 사용 할 때
select * from emp;
select * from dept;
...
-- allan 보다 높은 급여를 받는 사원 조회
select sal from emp where ename = 'allan';
select * from emp where sal > 1600;
select * from emp where sal > (select sal from emp where ename = 'allan');
...
-- 자동 번호 적용하기 (auto_increment는 pk 컬럼에만 적용가능)
...
create table member8 (
    ...
    member_password varchar(20) not null,
    created_at datetime default now(),
    -- id2 bigint auto_increment, -- 일반 컬럼에는 적용 불가
...
create table child1 (
    ...
    c2 varchar(20),
    p_id bigint, -- 참조할 컬럼
    -- 외래키(p_id 컬럼을 parent1 테이블의 id 컬럼을 참조하도록 함)
...
-- 부모 테이블의 데이터 삭제
-- id = 2인 데이터 한 줄을 삭제
-- 자식 테이블에 id = 2인 테이터를 참조하는 부분이 있기 때문에 삭제 불가
delete from parent1 where id = 2;
-- 자식 테이블에 id = 4인 테이터를 참조하는 부분이 없기 때문에 삭제 가능
...
create table child2 (
    ...
    c2 varchar(20),
    p_id bigint, -- 참조할 컬럼
    -- 외래키(p_id 컬럼을 parent2 테이블의 id 컬럼을 참조하도록 함)
    -- 부모 데이터 삭제시 자식 데이터도 삭제
...
create table child3 (
    ...
    c2 varchar(20),
    p_id bigint, -- 참조할 컬럼
    -- 외래키(p_id 컬럼을 parent3 테이블의 id 컬럼을 참조하도록 함)
    -- 부모 데이터 삭제시 자식 데이터도 삭제
...
insert into child3(id, c1, c2, p_id) values(4, 'ddd', 'ddd', 4);
select * from child3;
...
select sum(o_saleprice)
	from orders
		where customer_id in (select id from customer where c_name like '김%');
        
-- alter : 테이블의 구조를 변경 할 때(컬럼이름 변경, 타입 변경, 컬럼 삭제, 제약조건 추가 등)
...
update board_table set board_title = '안녕하십니까_수정', board_contents = '월요일 사라져라' where id = 4;
-- 5. 2번 회원이 자유게시판에 첫번째로 작성한 게시글 삭제 
delete from board_table where id = 5;
-- 7. 페이징 처리(한 페이지당 글 3개씩)
select * from board_table order by id desc;
select * from board_table order by id desc limit 0, 3; -- 18, 17, 16 => 1페이지
select * from board_table order by id desc limit 3, 3; -- 15, 14, 13 => 2페이지
select * from board_table order by id desc limit 6, 3; -- 12, 11, 10 => 3페이지
select * from board_table order by id desc limit 9, 3; -- 9, 8, 7 => 4페이지
select * from board_table order by id desc limit 12, 3; -- 4 => 5페이지

select * from board_table order by id desc limit 0, 5; -- 18, 17, 16, 15, 14
select * from board_table order by id desc limit 5, 5; -- 13, 12, 11, 10, 9
select * from board_table order by id desc limit 10, 5; -- 8, 7, 4
...
insert into comment_table(comment_writer, comment_comment, board_id, member_id)
	values('aa@aa.com', 'aa', '4', '1');
-- 1.2. 2번 회원이 1번 게시글에 댓글 작성 
insert into comment_table(comment_writer, comment_comment, board_id, member_id)
	values('bb@bb.com', 'aa', '4', '2');
-- 2. 댓글 조회
select * from board_table where id = 4;
select * from comment_table where board_id = 4;
select * from board_table b, comment_table c where b.id = c.board_id and b.id = 4;
-- 3. 댓글 좋아요 
...
select * from member_tbl_02 m, money_tbl_02 mo where m.custno = mo.custno;

drop table if exists tbl_member_202005;
create table tbl_member_202005 (
	m_mo char(1) not null primary key,
    m_name varchar(20),
    p_code char(2),
    p_school char(1),
    m_jumin char(13),
